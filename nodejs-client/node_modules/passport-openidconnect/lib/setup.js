...
exports.registration = function(provider, done) {
  ...
  (function pass(i, err, config) {
    ...
    try {
      var options = {};
      options.name = 'myoidc-test';
      options.redirectURI = [];
      options.redirectURI.push('https://localhost.airybox.org/auth/oidc/callback');
      var cb = layer(options, function(e, c) { pass(i + 1, e, c); } )
      cb(provider, function(e, c) { pass(i + 1, e, c); } );
    } catch (ex) {
      return done(ex);
    }
  })(0);
